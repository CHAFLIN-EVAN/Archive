...
import { Search, BookOpen, PlusSquare, Settings, Archive, FileText, Menu, X } from 'lucide-react';
  ...
  return (
    <>
      ...
      >
        ...
        {/* Nav items - 桌面端横向标签 */}
        <div
          className="bp-nav-desktop-items"
          style={{ display: 'flex', alignItems: 'stretch', height: '100%', gap: 0 }}
        >
          {NAV_ITEMS.map(item => {
            const active =
              item.to === '/'
        ...
        {/* 桌面端系统状态 */}
        <div
          className="bp-nav-desktop-status hidden lg:flex items-center gap-8"
        ...
        {/* 手机端汉堡菜单按钮 */}
        <button
          className="bp-nav-mobile-toggle items-center justify-center"
          style={{
            display: 'none',
            width: 42,
            height: 42,
            ...
            {NAV_ITEMS.map(item => {
              ...
              return (
                <Link
                  key={item.to}
                  to={item.to}
                  className={`bp-mobile-menu-item ${active ? 'active' : ''}`}
            ...
            >
              <div
                style={{
                  ...
                  background: 'radial-gradient(circle at 35% 35%, #50e880, #208840)',
                  border: '1px solid rgba(48,152,80,0.5)',
                  boxShadow: '0 0 6px rgba(48,200,80,0.5)',
                }}
              />
              <span
                style={{
